            ...
            if (success) {
              ...
              if (attackCount > 0) {
                ...
                for (let i=0;i<attackResults.length;i++) {
                  ...
                  const zone = getZoneFromD100(atkVal);
                  ...
                  const furyLogs = furyResult.furyLogs;
                  const dmgSum = dmgDice.reduce((a,b)=>a+b,0) + flat;
                  summary.push(`<div style="margin-left:12px">Dégâts (${dmgExpression}): ${dmgDice.join(', ')} ${flat? `+ ${flat}` : ''} → <strong>${dmgSum}</strong></div>`);
                  if (furyLogs.length) {
